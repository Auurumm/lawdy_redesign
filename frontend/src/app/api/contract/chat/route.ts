...
## 수집해야 할 정보
${Object.entries(fieldDescs).map(([key, desc]) => {
  const collected = currentExtractedData[key]?.trim();
  const isRequired = requiredFields.includes(key);
  return `- ${desc}${isRequired ? ' (필수)' : ' (선택)'}: ${collected ? `✅ "${collected}"` : '❌ 미수집'}`;
}).join('\n')}

## 현재 진행률: ${progress}%
## 미수집 필수 항목: ${missingFields.map(f => fieldDescs[f] || f).join(', ') || '없음 (모두 수집됨)'}
