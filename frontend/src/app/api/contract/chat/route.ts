...
isComplete는 모든 필수 항목이 수집되었을 때만 true로 설정하세요.`;
      ...
      try {
        ...
        if (jsonMatch) {
          ...
          const rawExtracted = parsed.extractedData || parsed.extracted_data || {};
          extractedData = normalizeExtractedData(rawExtracted, reverseMap);

          isComplete = parsed.isComplete === true || parsed.is_complete === true;
        }
      } catch {
        // JSON 파싱 실패시 - 응답 텍스트에서 직접 정보 추출 시도
        reply = rawContent.replace(/```[\s\S]*?```/g, '').trim() || rawContent;

        // 대화 내용에서 필드 값 직접 추출 시도
        const lastUserMsg = messages[messages.length - 1]?.content || '';
        for (const [key, desc] of Object.entries(fieldDescs)) {
          if (currentExtractedData[key]?.trim()) continue; // 이미 수집됨
          // 사용자 메시지에서 키워드 매칭으로 값 추출
          const descClean = desc.replace(/\(.*\)/, '').trim();
          const patterns = [
            new RegExp(`${descClean}[:\\s은는이가]\\s*(.+?)(?:[,\\n]|$)`, 'i'),
            new RegExp(`${key}[:\\s]\\s*(.+?)(?:[,\\n]|$)`, 'i'),
          ];
